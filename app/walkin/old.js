...
export default function WalkInPage() {
  ...
  // NEW: Handle customer form submission
  ...
  return (
    <div className={`h-screen flex ${classes.background} overflow-hidden transition-all duration-500`}>
      ...
      <div className={`w-64 ${classes.card} ${classes.shadow} shadow-xl ${classes.border} border-r flex flex-col`}>
        <div className={`p-4 ${classes.border} border-b ${classes.card}`}>
          ...
          <div className="flex items-center justify-between mb-2">
            <div>
              <h2 className={`text-xl font-bold ${classes.textPrimary}`}>Walk-In Order</h2>
            ...
            {categories.map((category) => {
              ...
              return (
                <motion.button
                  ...
                  className={`w-full text-left p-3 rounded-lg transition-all duration-300 group hover:${isDark ? 'bg-purple-900/20' : 'bg-purple-100'} ${isDark ? 'bg-gray-700/50' : 'bg-gray-50'}`}
                >
                  <div className="flex items-center">
                    <div className={`w-10 h-10 rounded-lg overflow-hidden mr-3 ${isDark ? 'bg-purple-900/30' : 'bg-purple-100'} flex items-center justify-center`}>
                      {category.image_url ? (
                        <img
                        ...
                        <IconComponent className={`w-5 h-5 ${isDark ? 'text-purple-400' : 'text-purple-600'}`} />
                      )}
                    </div>
                ...
                <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-3">
                  {categoryProducts.map((product) => (
                    ...
                    >
                      ...
                      <div className="p-2">
                        <h3 className={`font-bold ${classes.textPrimary} text-sm mb-1 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors truncate`}>
                          {product.name}
                        </h3>
                        {product.ingredients && (
      ...
      <div className={`w-80 ${classes.card} ${classes.shadow} shadow-xl ${classes.border} border-l flex flex-col`}>
        <div className={`p-4 ${classes.border} border-b ${classes.card}`}>
          <h2 className={`text-xl font-bold ${classes.textPrimary}`}>POS Walk-in</h2>
  ...
  className={`p-2 ${isDark ? 'bg-purple-600 hover:bg-purple-700' : 'bg-purple-500 hover:bg-purple-600'} rounded-lg transition-colors flex-shrink-0 shadow-md`}
>
  <Edit3 className="w-4 h-4 text-white" />
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() => setShowCustomerForm(true)}
              className={`w-full flex items-center justify-center p-4 ${classes.card} rounded-xl hover:${classes.shadow} hover:shadow-lg transition-all duration-200 ${classes.border} border-2 border-dashed hover:border-purple-500`}
            >
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1 min-w-0">
                        ...
                        {item.variantName && (
                          <p className={`text-xs ${isDark ? 'text-purple-400' : 'text-purple-600'} font-medium mt-0.5`}>
                            {item.variantName}
                          </p>
                        )}
                      </div>
        ...
        {cart.length > 0 && (
          <div className={`p-4 ${classes.border} border-t ${classes.card}`}>
            ...
            <motion.button
              ...
              className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200"
            >
      ...
      <AnimatePresence>
        {showVariantModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              {/* Header */}
              <div className="relative bg-gradient-to-r from-purple-600 to-purple-700 p-4 text-white flex-shrink-0">
                <button
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                {productVariants && productVariants.length > 0 && (
                  <div>
                    ...
                    <div className={`space-y-2 ${productVariants.length > 4 ? 'max-h-48 overflow-y-auto pr-1' : ''}`} style={{ scrollbarWidth: 'thin' }}>
                      {productVariants.map((variant) => {
                        ...
                        return (
                          <button
                            ...
                            className={`relative p-3 rounded-lg border-2 text-left transition-all duration-100 w-full ${isSelected
                              ? `border-purple-500 ${isDark ? 'bg-purple-900/20' : 'bg-purple-50'} shadow-md`
                              : `${classes.border} border-gray-200 hover:border-purple-300 hover:${isDark ? 'bg-gray-700/50' : 'bg-gray-50'}`
                              }`}
                          >
                            <div className="flex justify-between items-center">
                              ...
                              <div className="text-right ml-3 flex-shrink-0 flex items-center space-x-2">
                                <div className={`text-base font-bold ${isSelected
              ...
              <div className={`flex-shrink-0 p-4 ${classes.border} border-t ${isDark ? 'bg-gray-900/50' : 'bg-gray-50'}`}>
                <div className="flex space-x-3">
                  <button
                    onClick={() => setShowVariantModal(false)}
                    className={`flex-1 py-2.5 px-4 border-2 ${classes.border} ${classes.textPrimary} font-semibold rounded-lg hover:${isDark ? 'bg-gray-700' : 'bg-gray-100'} active:scale-98 transition-all text-sm`}
                  >
                    Cancel
                  </button>
                  ...
                  <button
                    ...
                    className={`flex-2 py-2.5 px-4 font-bold rounded-lg active:scale-98 transition-all duration-100 text-sm ${productVariants.length > 0 && !selectedVariant
