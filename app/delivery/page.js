...
export default function DeliveryPage() {
  ...
  useEffect(() => {
    const handleReloadCart = (event) => {
      ...
      if (savedOrderData) {
        ...
        setOrderData({ deliveryCharges: getDefaultDeliveryCharge() })
      }
    }
    ...
    try {
      ...
      if (newStatus === 'Preparing') {
        triggerWhatsAppAutoSend(order, user?.id, 'Preparing')
          .then(r => waNotify(r, 'WhatsApp: preparing notification sent to customer'))
          .catch(err => toast.error(`WhatsApp error: ${err.message}`, { duration: 5000 }))
      }
      if (newStatus === 'Ready') {
        triggerWhatsAppAutoSend(order, user?.id, 'Ready')
          .then(r => waNotify(r, 'WhatsApp: order ready notification sent to customer'))
          .catch(err => toast.error(`WhatsApp error: ${err.message}`, { duration: 5000 }))
      }
      if (newStatus === 'Dispatched') {
        triggerWhatsAppAutoSend(order, user?.id, 'Dispatched')
          .then(r => waNotify(r, 'WhatsApp: dispatch notification sent to customer'))
          .catch(err => toast.error(`WhatsApp error: ${err.message}`, { duration: 5000 }))
      }
      if (newStatus === 'Completed') {
  ...
  const handlePaymentRequired = async (order, paymentData) => {
    try {
      ...
      if (cacheManager.checkOnlineStatus() && paymentData.paymentMethod === 'Account' && order.customer_id) {
        ...
        console.log('💳 [Delivery Payment] Offline — Account ledger queued for sync')
      }
      ...
      const orderData = {
        ...
        delivery_boy_name: order.delivery_boys?.name || null,
        order: order
      }
  ...
  const handlePrintToken = async (order, loyaltyRedemption = null) => {
    ...
    try {
      ...
      const orderData = buildKitchenTokenPayload(order, mappedItems, { defaultOrderType: 'delivery' })
  ...
  const handleCustomerFormSubmit = (data) => {
    ...
    if (data.deliveryBoyId) {
      ...
      setOrderData(prev => ({ ...prev, deliveryBoyId: data.deliveryBoyId }))
    } else {
      localStorage.removeItem('delivery_boy_id')
      setOrderData(prev => { const next = { ...prev }; delete next.deliveryBoyId; return next })
    }
  ...
  const handleOrderAndPay = async () => {
    ...
    const effectiveDeliveryBoyName = effectiveDeliveryBoyId
      ? (cacheManager.getAllDeliveryBoys?.().find(b => b.id === effectiveDeliveryBoyId)?.name || null)
      : null
    ...
    const orderPayload = {
      ...
      delivery_boy_name: effectiveDeliveryBoyName,
    ...
    <ProtectedPage permissionKey="SALES_DELIVERY" pageName="Delivery Orders">
      ...
      <CartSidebar
        ...
        onInstructionsChange={(val) => {
          setOrderInstructions(val)
          setOrderData(prev => ({ ...prev, instructions: val }))
        }}
      />
