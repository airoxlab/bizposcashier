...
export default function TakeawayPage() {
    ...
    try {
      ...
      if (newStatus === 'Ready') {
        triggerWhatsAppAutoSend(order, user?.id, 'Ready')
          .then(r => waNotify(r, 'WhatsApp: order ready for pickup notification sent'))
          .catch(err => toast.error(`WhatsApp error: ${err.message}`, { duration: 5000 }))
      }
      if (newStatus === 'Completed') {
  ...
  const handlePrintToken = async (order, loyaltyRedemption = null) => {
    ...
    try {
      ...
      const orderData = buildKitchenTokenPayload(order, mappedItems, { defaultOrderType: 'takeaway' })
