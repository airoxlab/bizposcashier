...
export default function WalkinOrdersSidebar({
  ...
  const fetchTabCounts = useCallback(async () => {
    ...
    // Count only TODAY's active orders (business-day, overnight-safe) so the tab
    ...
    if (navigator.onLine && cacheManager.isOnline) {
      ...
      const counts = { walkin: 0, takeaway: 0, delivery: 0 }
      all.forEach(o => {
        if (ACTIVE.includes(o.order_status) && inDay(o) && counts[o.order_type] !== undefined) counts[o.order_type]++
      })
      setTabCounts(counts)
    }
  }, [showTypeTabs])
  ...
  const fetchPendingOrders = async () => {
    // Scope the active-orders list to the current BUSINESS day (overnight-safe cutoff),
    ...
    const inBusinessDay = (o) => {
      if (!o?.created_at) return true // keep rows without a timestamp (rare)
      const t = new Date(o.created_at).getTime()
      return t >= startMs && t < endMs
    }
  ...
  return (
    <div className={`w-64 h-full ${classes.card} ${classes.shadow} shadow-xl ${classes.border} border-r flex flex-col`}>
      ...
      <div className={`px-2 py-2 ${classes.border} border-b ${classes.card}`}>
        ...
        <div className="flex items-center justify-between gap-1.5 mb-0">
          <div className="min-w-0 flex-1">
            <h2 className={`text-base font-bold truncate ${classes.textPrimary}`}>
              New Order
            </h2>
          </div>
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            ...
            <motion.button
              ...
              className={`px-1.5 py-1 rounded-lg transition-all relative flex flex-col items-center gap-0 border ${
            ...
            >
              <ClipboardList className={`w-4 h-4 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
              <span className={`text-[9px] font-semibold leading-tight ${isDark ? 'text-blue-300' : 'text-blue-700'}`}>Orders</span>
            </motion.button>
            ...
            {orderType === 'walkin' && onTableClick && (
              ...
              >
                <Table2 className={`w-4 h-4 ${isDark ? 'text-green-400' : 'text-green-600'}`} />
                <span className={`text-[9px] font-semibold leading-tight max-w-[44px] truncate ${isDark ? 'text-green-300' : 'text-green-700'}`}>
      ...
      {showTypeTabs && showOrders && (
        <div className={`px-3 py-2.5 ${classes.border} border-b`}>
          <div className={`flex rounded-xl border ${isDark ? 'border-gray-600' : 'border-gray-200'}`}>
            {TYPE_TABS.map((tab, i) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex-1 flex flex-col items-center gap-0.5 py-1.5 px-1 text-[11px] font-semibold transition-all duration-200 ${
                    ...
                    i === TYPE_TABS.length - 1 ? 'rounded-r-[10px]' : ''
                  } ${
                ...
                >
                  ...
                  {count > 0 && (
                    <span className={`absolute -top-1.5 -right-1.5 z-10 min-w-[17px] h-[17px] px-1 rounded-full text-[9px] font-bold flex items-center justify-center bg-red-500 text-white shadow-sm border-2 ${isDark ? 'border-gray-800' : 'border-white'}`}>
                      {count > 99 ? '99+' : count}
                    </span>
                  )}
                </button>
