...
export default function CartSidebar({
  ...
  const [showOrderTakerDropdown, setShowOrderTakerDropdown] = useState(false)
  const orderTakerDropdownRef = useRef(null)
  const orderTakerButtonRef = useRef(null)

  // Close order taker dropdown on outside click (exclude the toggle button itself)
  useEffect(() => {
    if (!showOrderTakerDropdown) return
    const handler = (e) => {
      if (orderTakerButtonRef.current?.contains(e.target)) return
      if (orderTakerDropdownRef.current && !orderTakerDropdownRef.current.contains(e.target)) {
        setShowOrderTakerDropdown(false)
      }
    }
    document.addEventListener('mousedown', handler)
    return () => document.removeEventListener('mousedown', handler)
  }, [showOrderTakerDropdown])
      ...
      <div className={`p-2 ${classes.border} border-b ${isDark ? 'bg-gray-900/50' : 'bg-gray-50'}`}>
        <div className="flex items-center gap-1.5">
          ...
          {inlineCustomer && (
            customer ? (
              ...
              <div className="flex flex-1 items-center gap-1 min-w-0">
                <button
                  onClick={() => { setShowInstructionPanel(false); setShowOrderTakerDropdown(false); setCustMode(m => m === 'expanded' ? 'idle' : 'expanded') }}
          ...
          <motion.button
            ...
            title={orderInstructions ? 'Order note (added)' : 'Add note'}
          >
            <FileText className="w-3 h-3" />
            {/* Only show label when no customer selected — save space for customer name */}
            {!(inlineCustomer && customer) && <span>{orderInstructions ? 'Note ✓' : 'Note'}</span>}
            {inlineCustomer && customer && orderInstructions && <span className="w-1.5 h-1.5 rounded-full bg-current flex-shrink-0" />}
          </motion.button>

          {/* Order Taker Button — walkin only, in action row */}
          {orderType === 'walkin' && orderTakers.length > 0 && (
            <motion.button
              ref={orderTakerButtonRef}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              onClick={() => {
                setShowInstructionPanel(false)
                setCustMode('idle')
                setShowOrderTakerDropdown(v => !v)
              }}
              title={selectedOrderTaker ? `Order taker: ${selectedOrderTaker.name}` : (requireOrderTaker ? 'Select order taker (required)' : 'Select order taker')}
              className={`flex-shrink-0 flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                selectedOrderTaker
                  ? isDark ? 'bg-blue-900/30 border-blue-600 text-blue-300' : 'bg-blue-50 border-blue-400 text-blue-700'
                  : requireOrderTaker
                    ? isDark ? 'border-amber-600 border-dashed text-amber-400 bg-amber-900/10' : 'border-amber-400 border-dashed text-amber-600 bg-amber-50'
                    : isDark ? 'border-gray-600 text-gray-400 hover:border-blue-500 hover:text-blue-400' : 'border-gray-300 text-gray-500 hover:border-blue-400 hover:text-blue-600'
              }`}
            >
              {selectedOrderTaker ? (
                /* Show initials circle when selected — keeps button compact */
                <>
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold flex-shrink-0 ${isDark ? 'bg-blue-600 text-white' : 'bg-blue-600 text-white'}`}>
                    {selectedOrderTaker.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
                  </span>
                  <span
                    role="button"
                    onClick={e => { e.stopPropagation(); onOrderTakerChange?.(null); setShowOrderTakerDropdown(false) }}
                    className={`flex-shrink-0 p-0.5 rounded transition-colors ${isDark ? 'text-blue-400 hover:text-white' : 'text-blue-400 hover:text-blue-700'}`}
                  >
        ...
        {/* Order Taker Custom Dropdown */}
        {showOrderTakerDropdown && orderType === 'walkin' && orderTakers.length > 0 && (
          <div ref={orderTakerDropdownRef} className={`mt-1.5 rounded-lg border overflow-hidden shadow-lg ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className={`px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider ${isDark ? 'text-gray-500 bg-gray-900/40' : 'text-gray-400 bg-gray-50'}`}>
              {requireOrderTaker ? 'Select Order Taker *' : 'Order Taker (optional)'}
            </div>
            {selectedOrderTaker && (
              <button
                onClick={() => { onOrderTakerChange?.(null); setShowOrderTakerDropdown(false) }}
                className={`w-full flex items-center gap-2 px-2.5 py-2 text-xs transition-colors ${isDark ? 'text-gray-400 hover:bg-gray-700' : 'text-gray-400 hover:bg-gray-50'}`}
              >
                <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${isDark ? 'bg-gray-700' : 'bg-gray-100'}`}>
                  <X className="w-3 h-3" />
                </div>
                <span>Clear selection</span>
              </button>
            )}
            {orderTakers.filter(ot => ot.is_active !== false).map(ot => {
              const initials = ot.name ? ot.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) : '?'
              const isSelected = selectedOrderTaker?.id === ot.id
              return (
                <button
                  key={ot.id}
                  onClick={() => { onOrderTakerChange?.(ot); setShowOrderTakerDropdown(false) }}
                ...
                >
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-bold ${isSelected ? 'bg-blue-600 text-white' : isDark ? 'bg-gray-600 text-gray-300' : 'bg-gray-200 text-gray-600'}`}>
                    {initials}
                  </div>
                  <span className="flex-1 text-left font-medium">{ot.name}</span>
                  {isSelected && <UserCheck className="w-3 h-3 flex-shrink-0 text-blue-500" />}
                </button>
              )
            })}
          </div>
