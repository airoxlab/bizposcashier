...
export default function InlinePaymentSection({
  ...
  const DEFAULT_ACCT_UI = { icon: CreditCard, color: 'from-indigo-500 to-purple-600', requiresAmount: false, logo: null }
  ...
  const tillMethods = cashierAccounts.length > 0
    ? cashierAccounts.map(acct => {
        ...
        return { id: acct.payment_method_key || acct.id, name: acct.name, ...ui }
      })
    : [
  ...
  const paymentMethods = [
    ...tillMethods,
    { id: 'account', name: 'Account', displayName: 'Customer Account', icon: Users, color: 'from-purple-500 to-purple-600', requiresAmount: false, logo: null, requiresCustomer: true },
    { id: 'unpaid',  name: 'Unpaid',  icon: Clock,  color: 'from-gray-500 to-gray-600',  requiresAmount: false, logo: null },
    { id: 'complimentary', name: 'Complimentary', icon: Gift, color: 'from-pink-500 to-rose-600', requiresAmount: false, logo: null, requiresPermission: 'COMPLIMENTARY_ORDER', requiresReason: true },
