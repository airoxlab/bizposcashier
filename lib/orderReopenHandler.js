...
export class OrderReopenHandler {
  ...
  static async reopenOrder(order, orderItems, router) {
    try {
      // Subscription gate — reopening leads to re-punching, so block it too.
      if (planManager.isOrderBlocked && planManager.isOrderBlocked()) {
        return {
          success: false,
          error: 'Your BizPOS subscription has expired. Order taking is paused — please renew to continue.',
        }
      }

      console.log('📝 Reopening order:', order.order_number)

      // Only log to DB if this is a real synced order (UUID). Temp offline IDs
      // like "order_TIMESTAMP_abc" are not in the DB yet — the insert would fail
      // with a UUID parse error. The full history is written when the order syncs.
      ...
      if (isRealUUID) {
        ...
        console.log('📴 Skipping reopen log — order has temp offline ID, will sync when online')
      }
  ...
  static async updateExistingOrder(orderData, reopenData) {
    try {
      // Subscription gate — block saving order modifications once expired.
      if (planManager.isOrderBlocked && planManager.isOrderBlocked()) {
      ...
      // order_item_changes INSERT — one server-side transaction. Enforces
      // expected-order-number match and (by default) blocks modifying
      // Completed / Cancelled / already-Paid orders. This path is a reopen
      // flow that should respect the lock guard, so p_allow_locked = false.
      const currentUser = authManager.getCurrentUser?.()
      const cashierInfo = authManager.getCashierInfo?.()
      const p_order_data = {
      ...
      const p_items = orderData.cart.map(item => ({
      ...
      const { data: rpcResult, error: rpcError } = await supabase
        .rpc('modify_order_atomic', {
          p_order_id:              reopenData.originalOrderId,
          p_expected_order_number: reopenData.originalOrderNumber,
          p_order_data,
          p_items,
          p_changes: orderData.detailedChanges || {
            oldTotal: orderData.previousTotal,
            newTotal: orderData.total,
          },
          p_modifier_user_id:    currentUser?.id || null,
          p_modifier_cashier_id: cashierInfo?.id || null,
          p_modifier_name:       authManager.getDisplayName?.() || 'Staff',
          p_modifier_source:     'Desktop POS',
          p_notes:               `Order modified by ${authManager.getDisplayName?.() || 'Staff'} (${authManager.getRole?.()})`,
        })
