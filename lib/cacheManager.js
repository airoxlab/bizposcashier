  ...
  async initializeCache(forceRefresh = false) {
    try {
      ...
      if (this.isOnline && (this.isFirstLoad || forceRefresh)) {
        ...
        // Seed daily serial counter from DB so all terminals stay in sync
        if (this.userId) {
  ...
  try {
    ...
    if (order.id && isValidUUID(order.id)) {
      ...
      if (existingOrder && !checkError) {
        ...
        if (orderError) {
          // Unique constraint violation — another terminal inserted this order_number first
          // (race condition). Fall back to UPDATE using the now-existing record.
          if (orderError.code === '23505') {
            console.warn(`⚠️ [Sync] Race condition: ${order.order_number} was inserted by another terminal. Falling back to update.`)
            const { data: raceOrder, error: raceFetchError } = await supabase
              .from('orders')
              .select('id')
              .eq('order_number', order.order_number)
              .single()
            if (raceFetchError || !raceOrder) throw orderError
            order.id = raceOrder.id
            const { data: raceUpdate, error: raceUpdateError } = await supabase
              .from('orders')
              .update({ payment_method: order.payment_method, payment_status: order.payment_status, order_status: order.order_status })
              .eq('id', raceOrder.id)
              .select()
              .single()
            if (raceUpdateError) throw raceUpdateError
            syncedOrder = raceUpdate
            // Delete any items the winning terminal already inserted then reinsert ours
            await supabase.from('order_items').delete().eq('order_id', raceOrder.id)
          } else {
            throw orderError
          }
        } else {
    ...
    // Delete any existing items before inserting — prevents duplicate rows when two terminals
    // race to sync the same order (Terminal B inserts items while Terminal A is still awaiting
    // the assign_daily_serial RPC, then Terminal A also inserts items for the same order_id).
    await supabase.from('order_items').delete().eq('order_id', syncedOrder.id)
