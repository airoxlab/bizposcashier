  ...
  constructor() {
    ...
    if (typeof window !== 'undefined') {
      ...
      this.loadCacheFromStorage().then(() => {
        ...
        // Real connectivity check after storage loads — corrects any wrong initial value
...
async createOrder(orderData) {
  ...
  if (this.isOnline) {
    ...
    try {
      // Race syncOrder against a 8-second timeout to prevent indefinite hangs
...
async fetchAllData() {
  try {
    ...
    // Store require_order_taker setting to localStorage
    if (!userSettingsResult.error && userSettingsResult.data) {
      ...
      console.log(`📦 [Cache] require_order_taker setting: ${requireOrderTaker}`)
    }
  ...
  async refreshOrderTakerSettings() {
    ...
    try {
      ...
      if (!userSettingsResult.error && userSettingsResult.data && typeof window !== 'undefined') {
        ...
        console.log(`🔄 [Sync] require_order_taker refreshed: ${userSettingsResult.data.require_order_taker}`)
      }
    } catch (error) {
