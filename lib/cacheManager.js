...
async fetchAllData() {
  try {
    ...
    const [categoriesResult, productsResult, customersResult, dealsResult, tablesResult, menusResult, orderTakersResult, userSettingsResult, deliveryBoysResult] = await Promise.all([
      supabase
        .from('categories')
        .select('*')
        .eq('user_id', this.userId)
