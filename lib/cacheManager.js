...
class CacheManager {
  constructor() {
    ...
    this.cache = {
      ...
      defaultServiceCharge: { type: 'percentage', value: 0 }
    }

    this.userId = null
    this.initialized = false
    this.isFirstLoad = true
    // Per-order sync serialization: order_number → in-flight syncOrder promise.
    // createOrder's timed-out background sync, the syncOfflineData loop, the
    // manual offline-orders button and the modify path can all call syncOrder
    // for the SAME order — running them concurrently opens a SELECT-then-INSERT
    // race inside _syncOrderInner (double submit).
    this._inflightOrderSyncs = new Map()

    // Eagerly restore service + delivery charge defaults from localStorage so they
    // are available on the very first render — before initializeCache() runs
    // asynchronously. Without this, an order form that mounts during the startup
    // fetch window would read a missing key and silently show no charge.
    if (typeof window !== 'undefined') {
      try {
        const rawD = localStorage.getItem('pos_default_delivery_charge')
        if (rawD) {
        ...
        const rawS = localStorage.getItem('pos_default_service_charge')
        if (rawS) {
          const parsed = JSON.parse(rawS)
          if (parsed && parsed.value > 0) {
            this.cache.defaultServiceCharge = parsed
          }
        }
      } catch {}
    }
...
getAllCustomers() {
  ...
  getDefaultServiceCharge() {
    const mem = this.cache.defaultServiceCharge
    if (mem && mem.value > 0) return mem
    try {
      if (typeof window !== 'undefined') {
        const raw = localStorage.getItem('pos_default_service_charge')
        if (raw) {
          const parsed = JSON.parse(raw)
          if (parsed && parsed.value > 0) return parsed
        }
  ...
  // Resolve the configured default delivery charge to a concrete rupee amount.
  //   fixed      → the value as-is
  //   percentage → that percent of the supplied order subtotal
  // Callers that own the cart pass the live subtotal so percentage delivery
  // charges are honoured (previously they were silently dropped — only 'fixed'
  // was ever applied).
  resolveDeliveryChargeAmount(subtotal = 0) {
    const dc = this.getDefaultDeliveryCharge()
    if (!dc || !(dc.value > 0)) return 0
    if (dc.type === 'percentage') {
      const base = parseFloat(subtotal) || 0
      return Math.round(((base * dc.value) / 100) * 100) / 100
    }
    return dc.value
  }

  // Persist the service + delivery charge defaults (from a `users` settings row)
  // to both the in-memory cache and localStorage, then notify any mounted order
  // form to re-read them. Centralised so every sync path writes them identically.
  _persistChargeSettings(data) {
    if (typeof window === 'undefined' || !data) return
    const scType  = data.default_service_charge_type || 'percentage'
    const scValue = parseFloat(data.default_service_charge_value) || 0
    const dcType  = data.default_delivery_charge_type || 'fixed'
    const dcValue = parseFloat(data.default_delivery_charge_value) || 0
    this.cache.defaultServiceCharge  = { type: scType, value: scValue }
    ...
    try {
      window.dispatchEvent(new CustomEvent('pos-charge-settings-updated', {
        detail: {
          serviceCharge:  { type: scType, value: scValue },
          deliveryCharge: { type: dcType, value: dcValue },
        },
      }))
    } catch {}
  }

  // Lightweight, independent refresh of JUST the charge defaults (single-row
  // query). Safe to call on every order-screen mount: it can never be blocked by
  // an unrelated query failing (unlike fetchAllData's all-or-nothing Promise.all),
  // and it degrades gracefully offline by keeping the last cached values.
  async syncChargeSettings() {
    if (!this.userId || typeof window === 'undefined') return
    if (!this.isOnline) return
    try {
      const { data, error } = await supabase
        .from('users')
        .select('default_service_charge_type, default_service_charge_value, default_delivery_charge_type, default_delivery_charge_value, show_order_confirmation, auto_print_kitchen_token, auto_print_customer_receipt, auto_print_token_walkin, auto_print_token_takeaway, auto_print_token_delivery, auto_print_receipt_walkin, auto_print_receipt_takeaway, auto_print_receipt_delivery')
        .eq('id', this.userId)
        .single()
      if (error || !data) return
      this._persistChargeSettings(data)
      // Keep order behavior settings fresh without waiting for a full fetchAllData.
      const showOrderConf = data.show_order_confirmation !== false
      localStorage.setItem('pos_show_order_confirmation', JSON.stringify(showOrderConf))
      const legacyToken = !!data.auto_print_kitchen_token
      const legacyReceipt = !!data.auto_print_customer_receipt
      const tokenByType = {
        walkin: data.auto_print_token_walkin != null ? !!data.auto_print_token_walkin : legacyToken,
        takeaway: data.auto_print_token_takeaway != null ? !!data.auto_print_token_takeaway : legacyToken,
        delivery: data.auto_print_token_delivery != null ? !!data.auto_print_token_delivery : legacyToken,
      }
      const receiptByType = {
        walkin: data.auto_print_receipt_walkin != null ? !!data.auto_print_receipt_walkin : legacyReceipt,
        takeaway: data.auto_print_receipt_takeaway != null ? !!data.auto_print_receipt_takeaway : legacyReceipt,
        delivery: data.auto_print_receipt_delivery != null ? !!data.auto_print_receipt_delivery : legacyReceipt,
      }
      localStorage.setItem('pos_auto_print_token_by_type', JSON.stringify(tokenByType))
      localStorage.setItem('pos_auto_print_receipt_by_type', JSON.stringify(receiptByType))
    } catch (e) {
      console.warn('⚠️ [syncChargeSettings] failed:', e?.message)
    }
  }
  ...
  // Insert or replace a product in the in-memory + persisted cache so the POS
  // grid shows a just-created product without a full refetch. Pass `variants`
  // (array of product_variants rows) to seed the variants Map so the variant
  // picker works immediately for a product created with sizes/options.
  addProductToCache(product, variants = null) {
    if (!product) return
    if (!Array.isArray(this.cache.products)) this.cache.products = []
    const idx = this.cache.products.findIndex(p => p.id === product.id)
    if (idx >= 0) this.cache.products[idx] = product
    else this.cache.products.push(product)
    if (Array.isArray(variants)) {
      if (!(this.cache.variants instanceof Map)) this.cache.variants = new Map()
      if (variants.length > 0) this.cache.variants.set(product.id, variants)
      else this.cache.variants.delete(product.id)
    }
    this.saveCacheToStorage()
    if (typeof window !== 'undefined') {
  ...
  // Insert or replace a category in the in-memory + persisted cache.
  addCategoryToCache(category) {
    if (!category) return
    if (!Array.isArray(this.cache.categories)) this.cache.categories = []
    const idx = this.cache.categories.findIndex(c => c.id === category.id)
    if (idx >= 0) this.cache.categories[idx] = category
    else this.cache.categories.push(category)
    this.saveCacheToStorage()
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('pos-products-updated'))
    }
  ...
  // Toggle a product's or deal's business-wide favorite flag.
  // `entityType` is 'product' or 'deal'. Writes to Supabase (the same
  // products/deals rows the admin panel reads, so favorites stay in sync both
  // ways) and updates the in-memory + persisted cache so the POS grid reflects
  // it immediately. Reverts the in-memory change if the DB write fails.
  async toggleFavorite(entityType, id, nextValue) {
    const table = entityType === 'deal' ? 'deals' : 'products'
    const list = entityType === 'deal' ? this.cache.deals : this.cache.products

    const item = (list || []).find(x => x.id === id)
    const previousValue = item ? item.is_favorite : undefined
    if (item) item.is_favorite = nextValue
    this.saveCacheToStorage()
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('pos-favorites-updated'))
    }

    // Persist to Supabase. If offline, the in-memory change stands until the
    // next full cache refresh re-reads the (unchanged) server value.
    if (this.isOnline) {
      ...
      if (error) {
        console.error('[CacheManager] Failed to persist favorite:', error)
        if (item) item.is_favorite = previousValue
        this.saveCacheToStorage()
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('pos-favorites-updated'))
        }
  ...
  // Persist a manual product order for one category. `orderedIds` is the
  // category's product ids in their new on-screen order; each product's
  // sort_order is set to its index. Writes to the same products rows the admin
  // panel reads (so the arrangement stays in sync both ways) and updates the
  // in-memory + persisted cache so the POS grid reflects it immediately.
  // Reverts the in-memory change if the DB write fails.
  async reorderProducts(categoryId, orderedIds) {
    if (!Array.isArray(orderedIds) || orderedIds.length === 0) return

    // Snapshot previous sort_order values so we can revert on failure.
    const previous = new Map()
    orderedIds.forEach((id, index) => {
      const p = this.cache.products.find(x => x.id === id)
      if (p) {
        previous.set(id, p.sort_order)
        p.sort_order = index
      }
    })
    this.saveCacheToStorage()
    ...
    // Persist to Supabase. If offline, the in-memory change stands until the
    // next full cache refresh re-reads the (unchanged) server values.
    if (this.isOnline) {
      try {
        ...
        console.error('[CacheManager] Failed to persist product order:', err)
        // Revert every affected product to its snapshot sort_order.
        previous.forEach((sortOrder, id) => {
          const p = this.cache.products.find(x => x.id === id)
  ...
  // Fetch deal products from DB if not already cached for this deal.
  // Called before opening the deal selection screen so the cart always
  // gets a populated dealProducts array even after a stale-cache login.
  async ensureDealProducts(dealId) {
    if (this.cache.dealProducts.get(dealId)?.length > 0) return
    try {
      const { data, error } = await supabase
        .from('deal_products')
        .select('*')
      ...
      this.cache.dealProducts.set(dealId, [])
      data.forEach(dealProduct => {
        ...
        if (dealProduct.variant_id && productVariants.length > 0) {
          ...
          // Only pre-assign variant name when flavor change is not allowed; otherwise cashier picks
          if (matchedVariant && !dealProduct.allow_flavor_change) {
            variantName = matchedVariant.name
          }
        }
        this.cache.dealProducts.get(dealId).push({
          ...dealProduct,
          productName,
          variantName,
          matchedProduct,
...
async createOrder(orderData) {
  ...
  if (orderData.isModifying) {
    ...
    if (existingOrderIndex >= 0) {
      ...
      if (this.isOnline) {
        ...
        const modSyncResult = await this.syncOrder(updatedOrder)
        if (!modSyncResult?.success) {
          // The order was optimistically marked _isSynced above (isOnline was
          // true). If the direct sync failed, requeue it — otherwise this
          // modification would be silently lost.
          console.warn(`⚠️ [CacheManager] Direct modify sync failed (${modSyncResult?.error}) — queuing for offline replay`)
          updatedOrder._isSynced = false
          updatedOrder._syncError = modSyncResult?.error || 'modify sync failed'
          this.saveCacheToStorage()
        }
      } else {
  ...
  if (instantOnline) {
    ...
    // Keep a handle on the sync promise: Promise.race abandons it on timeout
    // but does NOT cancel the underlying HTTP insert — on a slow connection it
    // often still commits 15-30s later. The timeout catch below attaches a
    // continuation so the late success reconciles the cached offline copy
    // instead of leaving it queued for a pointless (and race-prone) re-sync.
    const syncPromise = this.syncOrder(orderTemplate)
    let syncTimeoutId
    ...
    try {
      ...
      const syncResult = await Promise.race([
        syncPromise,
        new Promise((_, reject) => { syncTimeoutId = setTimeout(() => reject(new Error('sync_timeout')), 10000) })
      ])
      ...
      if (syncResult && syncResult.success) {
        ...
        return { order: onlineOrder, orderNumber, dailySerial: dbSerial, inventoryDeduction: syncResult.inventoryDeduction || null }
      } else {
      ...
      if (error.message === 'sync_timeout') {
        // The abandoned syncOrder call is still in flight. If it eventually
        // succeeds, patch the cached copy with the real UUID and mark it
        // synced — otherwise the copy would replay through syncOfflineData
        // and (with the sidebar's offline list) show the order twice.
        syncPromise.then(lateResult => {
          if (!lateResult?.success || !lateResult.order?.id) return
          const idx = this.cache.orders.findIndex(
            o => o.order_number === orderNumber && !o._isSynced
          )
          if (idx === -1) return // already reconciled by a sync round
          this.cache.orders[idx] = {
            ...this.cache.orders[idx],
            id: lateResult.order.id,
            daily_serial: lateResult.order.daily_serial ?? this.cache.orders[idx].daily_serial,
            _isSynced: true,
            _isOffline: false,
            _syncedAt: new Date().toISOString(),
            _syncError: null,
          ...
          this.saveCacheToStorage()
          console.log(`🩹 [createOrder] Late sync completion reconciled cached copy of ${orderNumber}`)
        }).catch(() => {})
      }
 ...
 // Public entry point: serializes concurrent syncs of the same order.
 // If a sync for this order_number is already in flight, wait for it to finish
 // before running this one — the second pass then converges via the
 // existence-check (finds the row the first pass inserted → UPDATE, not INSERT).
 async syncOrder(order) {
  const key = order.order_number || order.id
  const prev = this._inflightOrderSyncs.get(key)
  const run = (async () => {
    if (prev) {
      console.log(`⏳ [Sync] Another sync in flight for ${key} — waiting for it first`)
      try { await prev } catch {}
    }
    return await this._syncOrderInner(order)
  })()
  this._inflightOrderSyncs.set(key, run)
  try {
    return await run
  } finally {
    if (this._inflightOrderSyncs.get(key) === run) this._inflightOrderSyncs.delete(key)
  }
 }

 async _syncOrderInner(order) {
  try {
    ...
    if (order.id && isValidUUID(order.id)) {
      ...
      // IMPORTANT: scoped to this user — the unique index on order_number is
      // GLOBAL across all tenants, so an unscoped lookup could match another
      // business's order and the modify path below would overwrite it.
      console.log(`🔍 [Sync] Checking if order ${order.order_number} already exists in database`)

      const orderOwnerId = order.user_id || this.userId
      ...
      if (checkError) {
        console.warn(`⚠️ [Sync] Existence check failed for ${order.order_number}: ${checkError.message} — treating as not found`)
      }
      ...
      if (existingOrder) {
        ...
        // On duplicate_order_number the RPC refuses to insert; we then either
        // converge to UPDATE (the row turned out to be ours — e.g. an earlier
        // timed-out attempt committed after the check above) or regenerate the
        // number (it belongs to a different order — generator collision).
        ...
        for (let attempt = 1; attempt <= 2 && !insertResult && !syncedOrder; attempt++) {
          const { data: res, error: insertError } = await supabase.rpc(
          ...
          if (res?.error === 'duplicate_order_number') {
            // Did OUR row land between the existence check and this insert
            // (an earlier abandoned/timed-out attempt committing late)?
            ...
            if (mine?.id) {
              console.log(`🔄 [Sync] ${order.order_number} was inserted by an earlier attempt — updating instead`)
              order.id = mine.id
              const { data: modResultC, error: modErrorC } = await supabase.rpc(
                'modify_order_atomic',
                buildAtomicParams(mine.id)
              )
              if (modErrorC) throw modErrorC
              if (!modResultC?.success) throw new Error(modResultC?.message || modResultC?.error || 'modify_order_atomic failed')
              syncedOrder = modResultC.order
              break
            }

            // The number is taken by a DIFFERENT order (weak-generator collision,
            // possibly another tenant). Never touch that row — mint a fresh number.
            if (attempt < 2) {
              const oldNumber = order.order_number
              order.order_number = this.generateOrderNumber()
              console.warn(`⚠️ [Sync] order_number collision: ${oldNumber} belongs to another order — regenerated as ${order.order_number}`)
              continue
            }
            throw new Error(`duplicate_order_number persisted after regenerating for ${order.order_number}`)
          }

          throw new Error(res?.error || 'insert_new_order_with_items failed')
        }
      ...
      if (orderTypeId && this.userId) {
        ...
        try {
          ...
          if (!deductionError && deductionResult && deductionResult.success) {
            ...
            inventoryDeduction = { attempted: true, success: true, deductionsMade: deductionResult.deductions_made || 0, totalCogs: deductionResult.total_cogs || 0 }
            ...
            if (errorMsg.includes('already deducted')) {
              ...
              const deductedIdx = this.cache.orders.findIndex(o => o.id === syncedOrder.id || o.order_number === order.order_number)
              if (deductedIdx !== -1) this.cache.orders[deductedIdx]._inventoryDeducted = true
              inventoryDeduction = { attempted: true, success: true, alreadyDeducted: true, deductionsMade: 0 }
            } else {
              console.warn(`⚠️ [Sync] Inventory deduction issue: ${errorMsg}`)
              inventoryDeduction = { attempted: true, success: false, error: errorMsg }
            }
          }
          ...
          inventoryDeduction = { attempted: true, success: false, error: inventoryError.message }
        }
      } else {
        ...
        inventoryDeduction = { attempted: false, success: false, error: 'Missing order_type_id or user' }
      }
    ...
    if (order.payment_method === 'Account' && customerId && order.order_status !== 'Cancelled') {
      try {
        ...
        if (debitAmount > 0) {
          // Single idempotent + race-safe RPC. It appends a debit for a new
          // order, no-ops if the same amount already exists, or updates +
          // recomputes the running balance if the order total changed (fixes
          // the old delete-and-reappend double-count bug).
          const { data: rpcData, error: rpcErr } = await supabase.rpc('post_customer_order_debit', {
            p_user_id: this.userId,
            p_customer_id: customerId,
            p_order_id: syncedOrder.id,
            p_amount: debitAmount,
            p_description: `Order #${order.order_number} - ${order.order_type?.toUpperCase() || 'WALKIN'}`,
            p_notes: order._isOffline ? `Synced from offline - ${order.items?.length || 0} items` : `${order.items?.length || 0} items`,
            p_created_by: this.userId,
          })

          const res = typeof rpcData === 'string' ? JSON.parse(rpcData) : rpcData
          if (rpcErr || !res?.success) {
            console.error('⚠️ [Sync] post_customer_order_debit failed:', rpcErr?.message || res?.error)
          } else {
            console.log(`✅ [Sync] Customer ledger debit (${res.action}) for order ${order.order_number}; balance: ${res.balance_after}`)
            if (res.action !== 'noop' && !order._wasOffline && typeof window !== 'undefined') {
              const { notify } = await import('../components/ui/NotificationSystem')
              const verb = res.action === 'adjusted'
                ? 'Updated customer account'
                : `Rs ${Number(debitAmount).toFixed(0)} added to customer account`
              notify.success(`${verb}. New balance: Rs ${Number(res.balance_after).toFixed(0)}`, { duration: 5000 })
            }
          }
        }
      } catch (ledgerError) {
    ...
    return { success: true, order: { id: syncedOrder.id, ...order }, inventoryDeduction }
  ...
  try {
    ...
    const [categoriesResult, productsResult, customersResult, dealsResult, tablesResult, menusResult, orderTakersResult, userSettingsResult, deliveryBoysResult, cashiersResult, paymentAccountsResult, expenseCategoriesResult, expenseSubcategoriesResult] = await Promise.all([
      ...
      supabase
        ...
        .eq('is_active', true)
        .eq('enable_for_pos', true)
        // Manual arrangement first (set in admin catalog), name as tie-breaker.
      ...
      supabase
        .from('users')
        .select('require_order_taker, require_table_walkin, default_service_charge_type, default_service_charge_value, default_delivery_charge_type, default_delivery_charge_value, require_customer_walkin, require_customer_takeaway, require_customer_delivery, toast_notifications_enabled, show_order_confirmation, auto_print_kitchen_token, auto_print_customer_receipt, auto_print_token_walkin, auto_print_token_takeaway, auto_print_token_delivery, auto_print_receipt_walkin, auto_print_receipt_takeaway, auto_print_receipt_delivery, use_cashier_drawer')
        .eq('id', this.userId)
        .single(),
      supabase
        .from('delivery_boys')
    ...
    if (!userSettingsResult.error && userSettingsResult.data) {
      ...
      if (typeof window !== 'undefined') {
        ...
        this._persistChargeSettings(userSettingsResult.data)

        // Require-table-on-walk-in flag. Enforced by walkin/ and new-order/
        // (Walk-in tab only) before an order can be punched. Default false.
        const requireTableWalkin = !!userSettingsResult.data.require_table_walkin
        localStorage.setItem('pos_require_table_walkin', JSON.stringify(requireTableWalkin))
        console.log(`📦 [Cache] require_table_walkin: ${requireTableWalkin}`)
        ...
        // Per-order-type auto-print maps. Fall back to the legacy global flag
        // when a per-type column is still null (row not yet migrated).
        const ud = userSettingsResult.data
        const autoPrintTokenByType = {
          walkin: ud.auto_print_token_walkin != null ? !!ud.auto_print_token_walkin : autoPrintKitchen,
          takeaway: ud.auto_print_token_takeaway != null ? !!ud.auto_print_token_takeaway : autoPrintKitchen,
          delivery: ud.auto_print_token_delivery != null ? !!ud.auto_print_token_delivery : autoPrintKitchen,
        }
        const autoPrintReceiptByType = {
          walkin: ud.auto_print_receipt_walkin != null ? !!ud.auto_print_receipt_walkin : autoPrintReceipt,
          takeaway: ud.auto_print_receipt_takeaway != null ? !!ud.auto_print_receipt_takeaway : autoPrintReceipt,
          delivery: ud.auto_print_receipt_delivery != null ? !!ud.auto_print_receipt_delivery : autoPrintReceipt,
        }
        localStorage.setItem('pos_auto_print_token_by_type', JSON.stringify(autoPrintTokenByType))
        localStorage.setItem('pos_auto_print_receipt_by_type', JSON.stringify(autoPrintReceiptByType))
        console.log(`📦 [Cache] order behavior: popup=${showOrderConf}, token=`, autoPrintTokenByType, 'receipt=', autoPrintReceiptByType)
    ...
    if (this.cache.deals.length > 0) {
      ...
      if (allDealProducts) {
        ...
        allDealProducts.forEach(dealProduct => {
          ...
          // Parse the product name from the deal product
          const productName = dealProduct.name
          ...
          // Resolve variant price and name via variant_id
          let variantPrice = null
          let variantName = null
          if (dealProduct.variant_id && productVariants.length > 0) {
            const matchedVariant = productVariants.find(v => v.id === dealProduct.variant_id)
            variantPrice = matchedVariant ? parseFloat(matchedVariant.price) : null
            // Pre-assign variant name only when flavor change is not allowed; otherwise cashier picks
            if (matchedVariant && !dealProduct.allow_flavor_change) {
              variantName = matchedVariant.name
            }
          }
...
async refreshTables() {
  ...
  async syncOfflineData() {
    ...
    try {
      ...
      if (successCount > 0) {
        ...
        // Announce that offline orders just landed in the DB. Views like the
        // Orders page and WalkinOrdersSidebar listen for 'ordersUpdated' and
        // re-fetch — this is the ONLY reliable post-sync refresh trigger when
        // the Supabase realtime socket didn't reconnect after the network blip
        // (it early-returns if offline at mount and doesn't re-subscribe).
        // Without it, freshly-synced orders can stay hidden until a manual
        // reload or the next day.
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('ordersUpdated', {
            detail: { source: 'offline-sync', count: successCount }
          }))
        }
      }
  ...
  async refreshOrderTakerSettings() {
    ...
    try {
      ...
      if (!userSettingsResult.error && userSettingsResult.data && typeof window !== 'undefined') {
        ...
        // Refresh require-table-on-walk-in flag
        localStorage.setItem('pos_require_table_walkin', JSON.stringify(!!userSettingsResult.data.require_table_walkin))
  ...
  async syncPendingLedgerEntries() {
    ...
    for (const entry of entries) {
      try {
        // Cancellation reversals replay through the idempotent reversal RPC.
        if (entry.type === 'reverse' && entry.order_id) {
          const { data: rpcData, error: rpcErr } = await supabase.rpc('reverse_customer_order_debit', {
            ...
            p_description: entry.description || 'Order cancellation - account credit reversal',
            p_created_by: this.userId,
          })
          const res = typeof rpcData === 'string' ? JSON.parse(rpcData) : rpcData
          if (rpcErr || !res?.success) {
            console.error('❌ [Sync] Failed to sync reversal:', rpcErr?.message || res?.error)
            remaining.push(entry)
            failedCount++
          } else {
            console.log(`✅ [Sync] Synced reversal (${res.action}) for order ${entry.order_id}`)
            successCount++
          }
          continue
        }

        // Order-linked debits go through the idempotent RPC so retries / dual
        // sync paths can never double-count and the running balance stays correct.
        if (entry.type === 'debit' && entry.order_id) {
          const { data: rpcData, error: rpcErr } = await supabase.rpc('post_customer_order_debit', {
            p_user_id: this.userId,
            p_customer_id: entry.customer_id,
            p_order_id: entry.order_id,
            p_amount: entry.amount,
            p_description: entry.description || 'Debit entry',
            p_notes: entry.notes || 'Synced from offline queue',
            p_created_by: this.userId,
          })
          const res = typeof rpcData === 'string' ? JSON.parse(rpcData) : rpcData
          if (rpcErr || !res?.success) {
            console.error(`❌ [Sync] Failed to sync ledger debit:`, rpcErr?.message || res?.error)
            remaining.push(entry)
            failedCount++
          } else {
            console.log(`✅ [Sync] Synced ledger debit (${res.action}): Rs ${entry.amount}`)
            successCount++
          }
          continue
        }

        // Non-order entries (e.g. cancellation credit reversals): append directly.
  ...
  generateOrderNumber() {
    // Full millisecond timestamp + 4 random digits. The old form kept only the
    // LAST 6 digits of the timestamp (repeats every ~16.7 minutes) + 3 random
    // digits — and order_number is globally unique across ALL businesses, so a
    // collision made sync silently target a different (even another tenant's)
    // order. Display code truncates with slice(-9), so the longer value is safe.
  ...
  enrichOrdersWithSerials(orders) {
    ...
    // Only today's orders participate in local serial assignment.
    const todaysOrders = orders.filter(order => toOrderBusinessDate(order) === todayBusiness)

    // STEP 1 — Reconcile the DB serials (authoritative) into localStorage FIRST, in
    // creation order. setSerial is collision-safe: it evicts any stale local guess that
    // was holding the same number, so two orders can never end up sharing a serial.
    const withDbSerial = todaysOrders
      .filter(o => o && o.order_number && o.daily_serial != null)
      .sort((a, b) => new Date(a.created_at || a.order_time || 0) - new Date(b.created_at || b.order_time || 0))
    withDbSerial.forEach(order => {
      dailySerialManager.setSerial(order.order_number, order.daily_serial)
    })

    // STEP 2 — Assign local numbers to offline orders that have NO DB serial yet and
    // no local serial yet, in creation order. Assignment is monotonic (always above the
    // current high-water mark) so a reset/drift can't reuse a number already on screen.
    const needsLocal = todaysOrders
      .filter(o => o && o.order_number && o.daily_serial == null && !dailySerialManager.getSerial(o.order_number))
      .sort((a, b) => new Date(a.created_at || a.order_time || 0) - new Date(b.created_at || b.order_time || 0))
    dailySerialManager.batchAssignSerials(needsLocal.map(o => o.order_number))

    console.log(`📊 [Serial Assignment] ${todaysOrders.length} today, ${withDbSerial.length} from DB, ${needsLocal.length} newly local-assigned`)

    // STEP 3 — Project the resolved serial onto each order.
    //   • Non-today orders: untouched (keep any DB serial).
    //   • Today + has DB serial: authoritative, keep as-is.
    //   • Today + no DB serial: use the localStorage value; if (and only if) there is
    //     none, leave the order UNCHANGED — never overwrite an existing serial with null,
    //     which is what made numbers momentarily blank ("disappear").
    return orders.map(order => {
      ...
      const local = dailySerialManager.getSerial(order.order_number)
      if (local == null) return order
      return { ...order, daily_serial: local }
    })
  }
