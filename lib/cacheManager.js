 ...
 async syncOrder(order) {
  try {
    ...
    if (order.id && isValidUUID(order.id)) {
      ...
      // Atomic UPDATE + replace all items via RPC
      const { data: upsertResultA, error: upsertErrorA } = await supabase.rpc(
      ...
      if (upsertErrorA) throw upsertErrorA
      if (!upsertResultA?.success) throw new Error(upsertResultA?.error || 'upsert_order_with_items failed')
      syncedOrder = upsertResultA.order
      console.log(`✅ [Sync] Updated existing order ${order.order_number} atomically`)
      ...
      if (existingOrder && !checkError) {
        ...
        // Atomic UPDATE + replace all items via RPC
        const { data: upsertResultB, error: upsertErrorB } = await supabase.rpc(
          'upsert_order_with_items',
          {
        ...
        if (upsertErrorB) throw upsertErrorB
        if (!upsertResultB?.success) throw new Error(upsertResultB?.error || 'upsert_order_with_items failed')
        syncedOrder = upsertResultB.order
        console.log(`✅ [Sync] Updated existing order ${order.order_number} atomically`)
        ...
        const { data: insertResult, error: insertError } = await supabase.rpc(
          'insert_new_order_with_items',
          {
            ...
            p_items: order.items,
            p_assign_daily_serial: !order._isOffline
          }
        )
        ...
        if (!order._isOffline && insertResult.daily_serial != null) {
          ...
          console.log(`🔢 [Sync] DB serial #${insertResult.daily_serial} assigned to ${order.order_number}`)
        } else {
