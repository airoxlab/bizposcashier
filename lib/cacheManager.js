...
async fetchAllData() {
  try {
    ...
    // Fetch categories, products, customers, deals, tables, and menus in parallel
    const [categoriesResult, productsResult, customersResult, dealsResult, tablesResult, menusResult] = await Promise.all([
      supabase
        .from('categories')
        .select('*')
        .eq('user_id', this.userId)
