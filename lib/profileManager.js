  ...
  async fetchProfileFromDatabase() {
    try {
      ...
      const { data, error } = await supabase
        .from('users')
        .select('id, customer_name, email, store_name, phone, store_address, store_logo, qr_code, invoice_status, hashtag1, hashtag2, show_footer_section, show_logo_on_receipt, show_business_name_on_receipt, show_dispatch_button, business_start_time, business_end_time')
        .eq('email', userEmail)
        .single()
