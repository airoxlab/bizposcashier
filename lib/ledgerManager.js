...
class LedgerManager {
  ...
  async recordPayment(userId, customerId, paymentData, receivedBy = null) {
    try {
      ...
      const ledgerResult = await this.createLedgerEntry(
        userId,
        customerId,
        'credit',
        amount,
      ...
      if (!ledgerResult.success) {
        console.error('❌ [recordPayment] Ledger entry failed — payment exists in customer_payments but NOT in customer_ledger:', ledgerResult.error);
        // Do NOT silently continue — throw so the caller knows
        throw new Error(`Payment saved but ledger entry failed: ${ledgerResult.error}`);
      }
      ...
      const { error: customerUpdateError } = await supabase
        .from('customers')
        .update({
          last_payment_date: localDateStr(),
