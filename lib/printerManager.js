...
class PrinterManager {
  ...
  async printKitchenTokens(orderData, userProfile, defaultPrinterConfig) {
    ...
    // Load category→printer mappings from Electron storage (or fall through to default)
    let mappings = []
    if (this.isElectron()) {
      try {
    ...
    console.log('[PrinterRouting] Mappings loaded:', JSON.stringify(mappings))

    // Load all printers from localStorage directly (always has Supabase UUIDs, bypasses printers.json)
    ...
    console.log('[PrinterRouting] Printers in localStorage:', configuredPrinters.map(p => `${p.name}(${p.id})`))
    console.log('[PrinterRouting] printerById keys:', Object.keys(printerById))
    ...
    console.log('[PrinterRouting] categoryToPrinter:', JSON.stringify(categoryToPrinter))
    console.log('[PrinterRouting] dealToPrinter:', JSON.stringify(dealToPrinter))

    // Group items by resolved printer ID ('default' means use defaultPrinterConfig)
    const groups = {} // printerId|'default' → [items]
    ...
    console.log('[PrinterRouting] Groups:', Object.keys(groups))

    // If no mappings resolved or all items in one group, fall through to single print
    const groupKeys = Object.keys(groups)
    if (groupKeys.length === 0) {
      // No items at all — still attempt a print with empty items
      return [await this.printKitchenToken(orderData, userProfile, defaultPrinterConfig)]
    }

    if (groupKeys.length === 1 && groupKeys[0] === 'default') {
      // All items routed to default → single print (existing behaviour)
      return [await this.printKitchenToken(orderData, userProfile, defaultPrinterConfig)]
    }

    // Multiple printers — send one token per printer
    const results = []
    for (const [printerId, items] of Object.entries(groups)) {
      ...
      const tokenData = { ...orderData, items }
      console.log(`[PrinterRouting] Sending ${items.length} item(s) to printer "${printerConfig.name || printerId}"`)

      try {
        const result = await this.printKitchenToken(tokenData, userProfile, printerConfig)
        results.push({ printerId, printerName: printerConfig.name, ...result })
      } catch (err) {
        results.push({ printerId, printerName: printerConfig.name, success: false, error: err.message })
      }
    }
