...
class PrinterManager {
  ...
  async printKitchenTokens(orderData, userProfile, defaultPrinterConfig) {
    ...
    // Nothing resolved at all (no items, no broadcast) → print to default
    // ONLY when there's no routing configured. If the user HAS configured
    // routing but no printer matched, respect that and print nothing.
    if (resolvedKeys.length === 0) {
      if (hasAnyRouting) {
        console.log('[PrinterRouting] No printer matched any items — skipping kitchen token print (routing configured but no match)')
        return []
      }
