...
class PrinterManager {
  ...
  async syncPrintersFromDatabase() {
    try {
      ...
      try {
        ...
        console.warn('Printer sync timed out or aborted — skipping')
        return false
      } finally {
      ...
      if (error) {
        ...
        if (msg) console.warn('Printer sync skipped:', msg)
        return false
      }
  ...
  async getPrinterForPrinting() {
    try {
      ...
      // If online and sync is needed, sync in background (don't wait, ignore errors)
      if (this.checkOnlineStatus() && this.needsSync()) {
        this.syncPrintersFromDatabase().catch(() => {})
      }
  ...
  // Fill receipt-display flags + footer/review messages from localStorage when
  // the caller didn't include them. Many callers build a slim userProfile that
  // omits these fields, which causes Electron-side defaults to kick in
  // (e.g. "Powered by airoxlab" always prints because the check is
  // `!== false` and `undefined !== false`). Centralizing the fallback here
  // means every page — orders, kds, walkin, takeaway, delivery, payment, etc. —
  // honors the same admin toggles without each page repeating the lookup.
  _augmentUserProfileFromCache(userProfile) {
    ...
    const resolveBool = (key, defaultVal = true) => {
      const candidates = [merged[key], cachedProfile?.[key], cachedUser?.[key]]
    ...
    // Display toggles — explicit boolean resolution from caller → user_profile → user
    merged.show_powered_by_airoxlab = resolveBool('show_powered_by_airoxlab')
    merged.show_footer_section = resolveBool('show_footer_section')
    merged.show_logo_on_receipt = resolveBool('show_logo_on_receipt')
    merged.show_business_name_on_receipt = resolveBool('show_business_name_on_receipt')

    // String fields — fall back through caller → user_profile → user, then ''
    const pickStr = (key) =>
      (merged[key] != null && merged[key] !== '') ? merged[key]
      : (cachedProfile?.[key] || cachedUser?.[key] || '')

    merged.receipt_review_message = pickStr('receipt_review_message')
    merged.receipt_footer_message = pickStr('receipt_footer_message')
    merged.hashtag1 = pickStr('hashtag1')
    merged.hashtag2 = pickStr('hashtag2')

    return merged
  }

  // Deep-clone via JSON to strip functions, class instances, and circular refs before Electron IPC
  _ipcSafe(obj) {
    // Recursive deep-clone that:
    //   • breaks true circular references (returns null for them)
    //   • preserves shared (non-circular) references by deleting from `seen` after processing
    //   • strips functions, symbols, and other non-IPC-safe values
    ...
    function clean(val) {
      ...
      if (seen.has(val)) return null   // true circular ref
      seen.add(val)
      let result
      if (Array.isArray(val)) {
        result = val.map(clean).filter(v => v !== undefined)
      } else {
        result = {}
      ...
      seen.delete(val)   // allow same object in sibling branches (shared ref, not circular)
      return result
    }
    return clean(obj)
  }
  ...
  async printReceipt(orderData, userProfile, printerConfig) {
    userProfile = this._augmentUserProfileFromCache(userProfile)
    orderData = this._ipcSafe(orderData)
    userProfile = this._ipcSafe(userProfile)
    printerConfig = this._ipcSafe(printerConfig)
      ...
      try {
        ...
        if (connectionType === 'windows_usb' || winPrinterNameFromPort) {
          ...
          return await window.electronAPI.printerPrintWindowsUSB(
            this._ipcSafe({ orderData, userProfile, printerConfig: { ...printerConfig, usb_printer_name: winPrinterName } })
          );
        }
        ...
        if (printerType === 'usb') {
          ...
          return await window.electronAPI.printerPrintUSB(
            this._ipcSafe({ orderData, userProfile, printerConfig: { ...printerConfig, usb_port: usbPort } })
          );
        } else {
          ...
          return await window.electronAPI.printerPrintReceipt(
            this._ipcSafe({ orderData, userProfile, printerConfig: { ...printerConfig, ip_address: ipAddress } })
          );
        }
      } catch (error) {
  ...
  // Print an end-of-shift cash report (Z-report). Mirrors printReceipt's
  // transport routing (network → Windows USB → USB → IP) but carries a
  // structured reportData that the Electron side renders via
  // generateCashReportESCPOS instead of the receipt generator.
  async printCashReport(reportData, userProfile, printerConfig) {
    userProfile = this._augmentUserProfileFromCache(userProfile)
    reportData    = this._ipcSafe(reportData)
    userProfile   = this._ipcSafe(userProfile)
    printerConfig = this._ipcSafe(printerConfig)

    // Network (client terminal) — hand off to the print server
    if (networkPrintManager.shouldUseNetwork()) {
      console.log('🌐 Network printing mode enabled - sending cash report to network')
      try {
        const printData = { reportData, userProfile, printerConfig, printType: 'cash_report' }
        const result = await networkPrintManager.sendPrintJobToNetwork(
          JSON.stringify(printData),
          this.currentUserId
        )
        if (result.success) {
          return { success: true, message: 'Cash report sent to network printer' }
        }
        return { success: false, error: result.error || 'Failed to send to network' }
      } catch (error) {
        console.error('❌ Network printing error for cash report:', error)
        return { success: false, error: error.message }
      }
    }
    ...
    try {
      ...
      // 0. Windows USB (USB001 / Devices & Printers) — check first
      const winFromPort   = hasUSB && usbPort.startsWith('WINUSB:') ? usbPort.replace(/^WINUSB:/, '') : null
      const winPrinterName = printerConfig.usb_printer_name || winFromPort
      if (connectionType === 'windows_usb' || winFromPort) {
        if (!winPrinterName) return { success: false, error: 'Windows USB printer selected but no printer name configured' }
        return await window.electronAPI.printerPrintWindowsUSBReport(
          this._ipcSafe({ reportData, userProfile, printerConfig: { ...printerConfig, usb_printer_name: winPrinterName } })
        )
      }

      // Resolve transport
      let printerType
      if (connectionType === 'usb') printerType = 'usb'
      else if (connectionType === 'ethernet' || connectionType === 'ip' || connectionType === 'network') printerType = 'ip'
      else if (hasUSB) printerType = 'usb'
      else if (hasIP) printerType = 'ip'
      else return { success: false, error: 'Printer configuration missing connection details' }

      if (printerType === 'usb') {
        if (!hasUSB) return { success: false, error: 'USB printer selected but no USB port configured' }
        return await window.electronAPI.printerPrintUSBReport(
          this._ipcSafe({ reportData, userProfile, printerConfig: { ...printerConfig, usb_port: usbPort } })
        )
      }
      if (!hasIP) return { success: false, error: 'IP printer selected but no IP address configured' }
      return await window.electronAPI.printerPrintReport(
        this._ipcSafe({ reportData, userProfile, printerConfig: { ...printerConfig, ip_address: ipAddress } })
      )
    } catch (error) {
      ...
      try {
        ...
        if (connectionType === 'windows_usb' || ktWinFromPort) {
          ...
          return await window.electronAPI.printerPrintWindowsUSBKitchen(
            this._ipcSafe({ orderData, userProfile, printerConfig: { ...printerConfig, usb_printer_name: ktWinPrinterName } })
          );
        }
        ...
        if (printerType === 'usb') {
          ...
          return await window.electronAPI.printKitchenToken(
            this._ipcSafe(orderData),
            this._ipcSafe(userProfile),
            this._ipcSafe({ ...printerConfig, ip_address: ipAddress })
          );
        }
      } catch (error) {
  ...
  /**
   * Print kitchen tokens with category/deal-based printer routing.
   * Items are grouped by their mapped printer. Items with no mapping
   * fall back to the default printerConfig passed in.
   * Returns an array of results, one per printer that was sent a job.
   */
  async printKitchenTokens(orderData, userProfile, defaultPrinterConfig) {
    console.log('[PrinterRouting] === printKitchenTokens START ===')

    // Load mappings from Electron storage
    let mappings = []
    if (this.isElectron()) {
      try {
        const res = await window.electronAPI.printerMappingsLoad()
        if (res.success) mappings = res.mappings || []
      } catch (e) {
        console.warn('[PrinterRouting] Could not load mappings:', e.message)
      }
    }

    // Separate broadcast printers (receive all items) from category/deal mappings
    const broadcastPrinterIds = mappings.filter(m => m.type === 'broadcast').map(m => m.printer_id)
    const routingMappings = mappings.filter(m => m.type !== 'broadcast')

    // Load all printers from localStorage directly (always has Supabase UUIDs)
    const configuredPrinters = this.getAllPrintersFromStorage()
      .filter(p => p.user_id === this.currentUserId)
    const printerById = {}
    configuredPrinters.forEach(p => { printerById[p.id] = p })

    // Build lookup: category_id → printerId, deal_id → printerId
    const categoryToPrinter = {}
    const dealToPrinter = {}
    routingMappings.forEach(m => {
      if (m.type === 'category') categoryToPrinter[m.id] = m.printer_id
      else if (m.type === 'deal') dealToPrinter[m.id] = m.printer_id
    })

    // Detect whether the user has configured ANY routing (broadcast OR specific
    // category/deal mappings). When routing IS configured, unmapped items
    // should NOT fall back to the default printer — only broadcast printers
    // receive items without explicit routing. Otherwise (no routing at all),
    // fall back to the default printer so first-time users still get tokens.
    const hasAnyRouting =
      broadcastPrinterIds.length > 0 ||
      Object.keys(categoryToPrinter).length > 0 ||
      Object.keys(dealToPrinter).length > 0

    // Group items by resolved printer ID
    const groups = {}
    ;(orderData.items || []).forEach(item => {
      ...
      if (item.isDeal && item.deal_id && dealToPrinter[item.deal_id]) {
        ...
        // Zero routing configured — fall back to default so tokens still print
        targetPrinterId = 'default'
      }
      // If no match and routing IS configured, item is "orphaned" — broadcast
      // printers will still receive it; otherwise it doesn't print anywhere.
      console.log(`[PrinterRouting] Item "${item.name}" category_id=${item.category_id} deal_id=${item.deal_id} → printer=${targetPrinterId || '(orphan — broadcast only)'}`)
      if (!targetPrinterId) return
      if (!groups[targetPrinterId]) groups[targetPrinterId] = []
      groups[targetPrinterId].push(item)
    })

    // Resolve groups to actual printerConfig objects, merge same physical printer
    const resolvedGroups = {} // resolvedId → { printerConfig, items[] }
    for (const [printerId, items] of Object.entries(groups)) {
      // When routing is configured, an unknown printer_id means a stale mapping —
      // skip it rather than silently dumping to the default printer.
      const printerConfig = printerId === 'default'
        ? defaultPrinterConfig
        : (printerById[printerId] || (hasAnyRouting ? null : defaultPrinterConfig))
      if (!printerConfig) continue
      const resolvedId = printerConfig?.id || 'default'
      if (!resolvedGroups[resolvedId]) {
        resolvedGroups[resolvedId] = { printerConfig, items: [] }
      }
      resolvedGroups[resolvedId].items.push(...items)
    }

    // Broadcast printers always receive ALL items — override any existing group for that printer
    for (const bpId of broadcastPrinterIds) {
      const bpConfig = printerById[bpId]
      if (!bpConfig) continue
      console.log(`[PrinterRouting] Broadcast printer "${bpConfig.name}" → all ${(orderData.items || []).length} items`)
      resolvedGroups[bpConfig.id] = { printerConfig: bpConfig, items: orderData.items || [] }
    }

    const resolvedKeys = Object.keys(resolvedGroups)

    // Nothing resolved at all → always fall back to the default printer so the
    // kitchen token is never silently lost. This covers two cases:
    //  a) No routing configured at all — straightforward default.
    //  b) Routing IS configured but no items matched any category/deal.
    //     Rather than swallowing the print job, we fall back to default so the
    //     cashier at least gets a token. Broadcast printers already run above.
    if (resolvedKeys.length === 0) {
      console.log('[PrinterRouting] No printer matched any items — falling back to default printer')
      return [await this.printKitchenToken(orderData, userProfile, defaultPrinterConfig)]
    }

    // Single printer → single print
    if (resolvedKeys.length === 1) {
      const { printerConfig, items } = resolvedGroups[resolvedKeys[0]]
      return [await this.printKitchenToken({ ...orderData, items }, userProfile, printerConfig)]
    }

    // Multiple printers — one token per printer
    const results = []
    for (const [, { printerConfig, items }] of Object.entries(resolvedGroups)) {
      console.log(`[PrinterRouting] Sending ${items.length} item(s) to "${printerConfig.name || printerConfig.id}"`)
      try {
        const result = await this.printKitchenToken({ ...orderData, items }, userProfile, printerConfig)
        results.push({ printerId: printerConfig.id, printerName: printerConfig.name, ...result })
      } catch (err) {
        results.push({ printerId: printerConfig.id, printerName: printerConfig.name, success: false, error: err.message })
      }
    }
    return results
  }

  // Live connection check for the dashboard status badge.
  // Pings the default printer (IP/USB/Windows USB) without printing anything.
  // Returns { connected, configured, printer, type, status, error }.
  async checkConnection() {
    try {
      if (!this.isElectron()) {
        return { connected: false, configured: false, error: 'desktop only' }
      }
      const printer = await this.getPrinterForPrinting()
      if (!printer) {
        return { connected: false, configured: false, error: 'no printer configured' }
      }
      const result = await window.electronAPI.printerCheckConnection(printer)
      // Persist last-known status so other parts of the app can read it
      if (printer.id) {
        try { await this.updateConnectionStatus(printer.id, !!result.connected) } catch {}
      }
      return {
      ...
      return { connected: false, configured: false, error: e.message }
    }
  }
