    ...
    try {
      ...
      for (const dbPrinter of printers) {
        ...
        const printerConfig = {
          id: dbPrinter.id,
          name: dbPrinter.name,
          printer_type: effectiveConnectionType === 'usb' ? 'usb' : (effectiveConnectionType === 'windows_usb' ? 'windows_usb' : 'ip'),
          connection_type: effectiveConnectionType,
          ip_address: dbPrinter.ip_address,
          port: dbPrinter.port || 9100,
          usb_port: usbPath,
          usb_device_path: usbPath,
          usb_printer_name: dbPrinter.usb_printer_name,
  ...
  async printReceipt(orderData, userProfile, printerConfig) {
    ...
    if (this.isElectron()) {
      try {
        ...
        if (connectionType === 'windows_usb') {
          const winPrinterName = printerConfig.usb_printer_name;
          if (!winPrinterName) {
    ...
    if (this.isElectron()) {
      try {
        ...
        if (connectionType === 'windows_usb') {
          ...
          console.log('Routing kitchen token to Windows USB printer:', winPrinterName);
