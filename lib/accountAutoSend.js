...
import { getStatus as waGetStatus, enqueueTextAndWait, enqueueImageAndWait } from './whatsappQueue'
...
export async function triggerAccountAutoSend({
  ...
  try {
    // Desktop is needed only to RENDER the receipt image locally; text sends
    // work anywhere. The actual WhatsApp send happens on the server.
    ...
    // 2. Check WhatsApp connection (centralized — server-side socket)
    ...
    // 6. Send via WhatsApp (centralized queue). Image is rendered locally then
    //    uploaded; if we can't render (non-desktop), fall back to text.
    let result
    if (settings.send_receipt_image && canRenderImage) {
      ...
      if (rendered?.success && rendered.dataUrl) {
        result = await enqueueImageAndWait({ restaurantId: userId, phone, message, dataUrl: rendered.dataUrl, filename: `receipt_${orderNumber || 'order'}.png` })
      } else {
