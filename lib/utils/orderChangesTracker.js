...
// Increment the version counter for an order — call ONCE when order is saved with changes.
export function incrementUpdateVersion(orderId) {
  if (typeof window === 'undefined') return null
  const counts = JSON.parse(localStorage.getItem('pos_update_token_count') || '{}')
  const next = (counts[orderId] || 0) + 1
  counts[orderId] = next
  localStorage.setItem('pos_update_token_count', JSON.stringify(counts))
  return String.fromCharCode(64 + Math.min(next, 26))
}

// Reset all change-tracking for an order — call when the order is converted to
// another type. The order id stays the same across a conversion, so without this
// the stale diff (+N markers) and update-version letter would bleed into the new
// type. After this runs, the order prints clean merged quantities with no
// "changes/updated" annotations, as a fresh hand-off to the new station.
export function clearOrderChangeTracking(orderId) {
  if (typeof window === 'undefined' || !orderId) return
  try {
    const counts = JSON.parse(localStorage.getItem('pos_update_token_count') || '{}')
    if (orderId in counts) {
      delete counts[orderId]
      localStorage.setItem('pos_update_token_count', JSON.stringify(counts))
    }

    const cached = JSON.parse(localStorage.getItem('order_changes') || '{}')
    if (orderId in cached) {
      delete cached[orderId]
      localStorage.setItem('order_changes', JSON.stringify(cached))
    }

    console.log('🧹 [Changes] Cleared change tracking for converted order:', orderId)
  } catch (error) {
    console.error('❌ [Changes] Error clearing change tracking:', error)
  }
}

// Read the current version letter without incrementing — call from print handlers.
export function getCurrentUpdateVersion(orderId) {
  if (typeof window === 'undefined') return null
  const counts = JSON.parse(localStorage.getItem('pos_update_token_count') || '{}')
  const current = counts[orderId] || 0
  return current > 0 ? String.fromCharCode(64 + Math.min(current, 26)) : null
}
  ...
  try {
    ...
    if (detailedChanges.itemsAdded) {
      detailedChanges.itemsAdded.forEach(item => {
        ...
        if (item.isDeal && item.dealProducts) entry.deal_products = item.dealProducts
        changes.push(entry)
      })
    }
    ...
    if (changes.length > 0) {
      ...
      cacheOrderChanges(orderId, changes)
      // Increment version counter here — this is the single authoritative call site
      incrementUpdateVersion(orderId)
      ...
      try {
        ...
        const { data: historyData, error: historyError } = await supabase
          ...
          .insert({
            ...
            changes: { offline_sync: true, changesCount: item.changes.length },
            actor_name: null,
