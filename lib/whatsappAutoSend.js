...
export async function triggerWhatsAppAutoSend(order, userId, triggerStatus = 'Completed') {
  ...
  try {
    ...
    // ── 5. Get customer ───────────────────────────────────────────────────
    // Checked before the (slower) WhatsApp connection check below so orders
    // with no customer attached fail fast instead of burning a connection retry.
    ...
    // ── 6. Check WhatsApp connection (centralized — server-side socket) ────
