...
export async function triggerWhatsAppAutoSend(order, userId, triggerStatus = 'Completed') {
  ...
  try {
    ...
    if (!isElectron) {
      await logResult({ ...logBase, status: 'skipped', skip_reason: 'not_electron' }); return { success: false, silent: true }
    }
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ;({ status: waStatus } = await window.electronAPI.whatsapp.getStatus())
      if (waStatus === 'connected') break
      if (waStatus !== 'connecting') break  // 'disconnected' or 'qr_ready' — won't self-heal
