  ...
  async login(phone, password) {
    try {
      ...
      if (adminUser && !adminError && adminUser.password === password) {
        ...
        // Apply this admin's layout theme preference
        if (typeof window !== 'undefined') {
        ...
        // Force-refresh plan on login so stale cache never survives a plan change
        planManager.refresh(adminUser.id).catch(err =>
          console.warn('⚠️ Plan refresh failed (non-blocking):', err)
        )

        // Disconnect WhatsApp only if the restaurant changed (non-blocking)
        if (typeof window !== 'undefined' && window.electronAPI?.whatsapp?.onUserLogin) {
          window.electronAPI.whatsapp.onUserLogin({ restaurantId: adminUser.id }).catch(() => {})
        }
      ...
      if (cashier && !cashierError && cashier.password === password) {
        ...
        // Apply this cashier's own layout theme preference
        if (typeof window !== 'undefined') {
        ...
        // Disconnect WhatsApp only if the restaurant changed (non-blocking)
        if (typeof window !== 'undefined' && window.electronAPI?.whatsapp?.onUserLogin) {
          window.electronAPI.whatsapp.onUserLogin({ restaurantId: cashier.user_id }).catch(() => {})
        }
