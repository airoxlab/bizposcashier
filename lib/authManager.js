...
async getOrderHistory(orderId) {
  ...
  async logout() {
    try {
      // Disconnect WhatsApp so the next user doesn't inherit this account's WA session
      if (typeof window !== 'undefined' && window.electronAPI?.whatsapp) {
        try {
          await window.electronAPI.whatsapp.disconnect()
          console.log('📱 WhatsApp disconnected on logout')
        } catch (err) {
          console.log('⚠️ WhatsApp disconnect skipped:', err?.message)
        }
      }
