...
class AuthManager {
  ...
  async login(phone, password) {
    try {
      ...
      // Query admin and cashier tables in parallel — cuts round-trip from 2 serial
      // fetches to 1 simultaneous fetch. Whichever matches wins.
      const [adminResult, cashierResult] = await Promise.all([
        supabase.from('users').select('*').eq('phone', cleanPhone).eq('is_active', true).single(),
        supabase.from('cashiers').select(`
          *,
          user:user_id (
            ...
            show_business_name_on_receipt,
            show_powered_by_airoxlab,
            show_dispatch_button,
            business_start_time,
            business_end_time,
            kds_new_order_sound,
            kds_order_timeout_minutes,
            kds_timeout_sound_enabled,
            use_cashier_drawer,
            receipt_review_message,
            receipt_footer_message
          )
        `).eq('phone', cleanPhone).eq('is_active', true).single()
      ])

      const adminUser = adminResult.data
      const adminError = adminResult.error
      const cashier = cashierResult.data
      const cashierError = cashierResult.error

      // --- ADMIN path ---
      if (adminUser && !adminError && adminUser.password === password) {
        console.log('✅ Admin user authenticated:', adminUser.customer_name)

        // Session creation is audit-only — fire non-blocking so it never delays login
        let sessionRef = null
        this.createSession(null, adminUser.id, 'admin').then(s => {
          sessionRef = s
          // Patch the stored auth state with the real session id once it arrives
          try {
            const stored = this.getAuthStateSync()
            if (stored) { stored.session = s; this.saveAuthState(stored) }
          } catch {}
        }).catch(err => console.warn('⚠️ Session creation failed (non-blocking):', err))
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('pos_layout_theme', adminUser.layout_theme || 'classic')
        }
        ...
        planManager.refresh(adminUser.id).catch(err =>
          console.warn('⚠️ Plan refresh failed (non-blocking):', err)
        )

        // WhatsApp is now a centralized server-side socket (see lib/whatsappQueue).
        // No per-PC connect/disconnect on login — the shared session persists.

        console.log('✅ Admin login successful')
        return { success: true, user: adminUser, session: null, role: 'admin' }
      }
      ...
      if (cashier && !cashierError && cashier.password === password) {
        ...
        // Session creation is audit-only — fire non-blocking
        this.createSession(cashier.id, cashier.user_id, 'cashier').then(s => {
          try {
            const stored = this.getAuthStateSync()
            if (stored) { stored.session = s; this.saveAuthState(stored) }
          } catch {}
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('pos_layout_theme', cashier.layout_theme || 'classic')
        }
        ...
        planManager.refresh(cashier.user_id).catch(err =>
          console.warn('⚠️ Plan refresh failed (non-blocking):', err)
        )
        ...
        return { success: true, user: userData, cashier, session: null, role: 'cashier' }
      }
...
async logOrderAction(orderId, actionType, changes = null, notes = null) {
  try {
    ...
    const historyEntry = {
      ...
      actor_name: cashier?.name || currentUser?.customer_name || null,
    ...
    if (changes && historyData) {
      ...
      if (itemChanges.length > 0) {
        ...
        if (itemChangesError) {
          console.error('Failed to log item changes:', JSON.stringify(itemChangesError), 'Data:', JSON.stringify(itemChanges))
        } else {
...
async getOrderHistory(orderId) {
  ...
  loadAuthState() {
    try {
      ...
      // Re-apply layout theme so pos_layout_theme is always in sync with the logged-in account
      const restoredTheme = (authData.role === 'cashier' && authData.cashier?.layout_theme)
        ? authData.cashier.layout_theme
        : (authData.user?.layout_theme || 'classic')
      localStorage.setItem('pos_layout_theme', restoredTheme)

      // Trigger plan load on every app restore:
      // • Cache hit → keeps isLoaded=true while silently refreshing from server;
      //   picks up plan changes and updates localStorage.
      // • Cache miss (first load / expired) → fetches fresh and unblocks all PlanGate components.
      // • Offline → load() falls back to cache automatically.
      const userId = authData.user?.id
      if (userId) {
        planManager.load(userId).catch(err =>
          console.warn('⚠️ Plan auto-load on restore failed (non-blocking):', err)
        )
      }
  ...
  // ==============================================
  // LAYOUT THEME (per-user / per-cashier)
  // ==============================================

  getLayoutTheme() {
    if (this.currentRole === 'cashier' && this.cashierData) {
      return this.cashierData.layout_theme || 'classic'
    }
    return this.currentUser?.layout_theme || 'classic'
  }

  async saveLayoutTheme(theme) {
    try {
      if (this.currentRole === 'cashier' && this.cashierData) {
        const { error } = await supabase
          .from('cashiers')
          .update({ layout_theme: theme })
          .eq('id', this.cashierData.id)
        if (error) throw error
        this.cashierData = { ...this.cashierData, layout_theme: theme }
        const authState = this.getAuthStateSync()
        if (authState) {
          authState.cashier = this.cashierData
          this.saveAuthState(authState)
        }
      } else {
        const { error } = await supabase
        ...
        if (error) throw error
        this.currentUser = { ...this.currentUser, layout_theme: theme }
        const authState = this.getAuthStateSync()
        if (authState) {
          authState.user = this.currentUser
          this.saveAuthState(authState)
      ...
      console.error('❌ Failed to save layout theme:', error)
      return { success: false, error: error.message }
    }
  }
  ...
  clearAuth() {
    ...
    if (typeof window !== 'undefined') {
      // Wipe ALL user-specific localStorage so the next account starts completely fresh.
      // localStorage.clear() is the safest approach — every key here is either
      // user-scoped (auth, cache, cart, order state) or can be re-fetched on next login.
      // Machine-level config (backup folder path, printer IP) lives in Electron's
      // userData folder (via IPC), not in localStorage, so it is unaffected.
      localStorage.clear()

      console.log('🔒 Auth state cleared — localStorage wiped')
    }
  }

  async logout() {
    try {
      // WhatsApp is NOT disconnected here — it stays connected so that the next
      // login from the same restaurant can keep using it. If the next login is
      // from a different restaurant, the whatsapp:on-user-login handler will
      // disconnect it at that point.
