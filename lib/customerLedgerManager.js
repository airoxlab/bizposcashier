...
class CustomerLedgerManager {
  ...
  async getCustomerBalance(customerId) {
    ...
    try {
      ...
      // SINGLE SOURCE OF TRUTH: customers.account_balance (maintained atomically by
      // every write RPC). We do NOT re-derive from customer_ledger.balance_after —
      // that is non-deterministic when rows share a timestamp (tie → arbitrary row),
      // which made the cart and the reports disagree.
      ...
      const balance = Number(data?.account_balance || 0)
